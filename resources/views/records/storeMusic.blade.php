...
@section('title', '| Musica Grabaciones')
...
<div class="row">
	<div class="col-md-6 col-md-offset-3">
		...
		<h4 class="text-center" style="color:#EAE4CC">{{$record->disc}}</h4>
		<hr>

		{!!Form::open(['action' => ['MusicRecordController@storeMusic', $record->id], 'method' => 'POST'])!!}

			<input type="hidden" name="id_music" value="{{$record->id}}">
			<div class="row">
				<div class="col-md-8">
			...
			<div class="row" style="margin-top: 25px;">
				...
				<div class="col-md-6">
					<a href=" {{url('admin/grabaciones')}} " class="btn btn-info btn-sm btn-block"> Volver a Grabaciones</a>
				</div>
			</div>
	...
	<div class="row" style="margin-top: 45px;">
		...
		@if(count($musicsRecord) >= 1)
			<table class="table">
				...
				<tbody>
				@foreach($musicsRecord as $musicRecord)
					<tr>
						<td style="font-weight: 600" class="td_music"> {{$musicRecord->name_spotify}} </td>
						<td style="font-weight: 600" class="td_music">
							@if(!empty($musicRecord->link_spotify))
						...
						<td style="font-weight: 600" class="td_music"> {{$musicRecord->name_soundcloud}} </td>
						<td style="font-weight: 600" class="td_music">
							@if(!empty($musicRecord->link_soundcloud))
								<img src="{{asset('img/ok.png')}}" class="img-responsive center-block" style="width: 25px;">
							@else
								<img src="{{asset('img/no.png')}}" class="img-responsive center-block" style="width: 25px;">
							@endif
						...
						<td>
							<a href=" {{action('MusicRecordController@getUpdate', [$record->id])}} "><span class="glyphicon glyphicon-edit" aria-hidden="true" style="color:#b9b9b9; font-size: 16px; margin-top: 8px;"></span></a>
						</td>
						<td>
							{!!Form::open(array('action' => array( 'MusicRecordController@destroyMusic', $musicRecord->id), 'method' => 'DELETE', 'style' => 'text-align:center','id' => 'formPrevent'))!!}
			...
			@else
				<div class="alert alert-warning text-center"><strong>Todavía no agregaste música relativa a <span style="text-decoration: underline;">{{$record->disc}}</span>!</strong></div>
			@endif
		</div>
	</div>
